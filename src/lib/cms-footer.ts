...
export async function getSiteFooterContent(): Promise<SiteFooterContent> {
  ...
  return {
    columns: [productLinks, solutionLinks].filter((column): column is FooterColumn => Boolean(column)).length
      ? [productLinks, solutionLinks].filter((column): column is FooterColumn => Boolean(column))
      : footerColumns,
    description:
